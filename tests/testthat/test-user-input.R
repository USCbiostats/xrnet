...
test_that("throw error when x or ext do not have at least 2 columns", {
    ...
    expect_error(hierr(x, y, external, "gaussian"), "Error: Both x and external must have at least 2 columns")

    x <- matrix(runif(10), ncol = 2)
    y <- 1:5
    external <- matrix(runif(2), ncol = 1)
    expect_error(hierr(x, y, external, "gaussian"), "Error: Both x and external must have at least 2 columns")
})
...
test_that("throw error when dimensions of x and y do not match", {
    ...
    y <- 1:10
    external <- matrix(runif(10), nrow = 2)
    expect_error(hierr(x, y, external, "gaussian"), "Error: Number of observations in y (10) not equal to the number of rows of x (5)", fixed = TRUE)
})
