...
test_that("predict returns estimates for penalties already fit by hierr object", {
    hierr_object <- hierr(x, y, z, family = "gaussian", penalty = definePenalty(penalty_type = 0, penalty_type_ext = 1, user_penalty = c(2, 1, 0.05), user_penalty_ext = c(0.2, 0.1, 0.05)))
...
test_that("predict returns estimates for penalties notfit by hierr object", {
    hierr_object <- hierr(x, y, z, family = "gaussian", penalty = definePenalty(penalty_type = 0, penalty_type_ext = 1, user_penalty = 1:20, user_penalty_ext = seq(0.01, 0.1, 0.01)), control = list(tolerance = 1e-20))
