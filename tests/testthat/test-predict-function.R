...
test_that("predict returns estimates for penalties already fit by hierr object", {
    ...
    test <- predict(hierr_object, penalty = 1, penalty_ext = 0.05, type = "coefficients")
    expect_identical(drop(predict(hierr_object, penalty = 1, penalty_ext = 0.05, type = "coefficients")$betas), hierr_object$betas[, 2, 3])
