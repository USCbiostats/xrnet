...
The hierr R package is an extension of regularized regression (i.e. ridge regression) that enables the incorporation of external data that may be informative for the effects of the predictors on an outcome of interest. Let $y\in\mathbb{R}^n$ be the observed outcome vector, $X\in\mathbb{R}^{n \times p}$ the set of p predictors observed for the n observations, and $Z\in\mathbb{R}^{p \times q}$ the set of q external features available for the p predictors. We extend the standard two-stage regression model:
...
where $\beta$ is a $p \times 1$ vector of coefficients describing the association of each predictor with the outcome and $\alpha$ is a $q \times 1$ vector of coefficients describing the association of each external feature with the predictor coefficients, $\beta$. Rewriting this convex optimization with the variable subsitution $\gamma = \beta - Z$ $\alpha$, we see that this is a generalization of standard regularized regression in which we allow the penalty value ($\lambda_1$ / $\lambda_2$) and type (ridge / lasso) to be variable-specific:
