...
The hierr R package is an extension of regularized regression (i.e. ridge regression) that enables the incorporation of external data that may be informative for the effects of the predictors on an outcome of interest. Let $y\in\mathbb{R}^n$ be the observed outcome vector, $X\in\mathbb{R}^{n \times p}$ be the set of p predictors observed from n observations, and $Z\in\mathbb{R}^{p \times q}$ be the set of q external features available for the p predictors. Assuming that the outcome is continuous, we minimize the following objective function:

$$\min_{\beta,\alpha}||y - X\beta||^2_2$$
